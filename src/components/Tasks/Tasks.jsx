...
const Tasks = ({ activeItem, onEditTitle, onAddTask, onRemoveTasks }) => {
  //Изменение заголовка
  const editTitle = () => {
  ...
  return (
    <div className="tasks">
      {
        //Заголовок
        <h2 style={{ color: activeItem.color.hex }} className="tasks__title">
          {activeItem.name}
          <svg
            onClick={() => editTitle()}
            width="15"
...

      {
        ...
        <div className="tasks__items">
          {!activeItem.tasks.length ? (
            <div>
              <h2>Задачи отсутствуют</h2>
            </div>
          ) : (
            activeItem.tasks.map((item) => {
              //Задача
              return (
                <Task onRemoveTasks={onRemoveTasks} key={item.id} item={item} />
              );
            })
          )}
        </div>
...

      {
        ...
        <AddTasks activeItem={activeItem} onAddTask={onAddTask} />
      }
    </div>
  );
};
