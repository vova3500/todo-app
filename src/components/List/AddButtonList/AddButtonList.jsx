...
const AddButtonList = ({ colors, onAdd }) => {
  //Состояние окна добавления списка задач
  const [visiblePopup, setVisiblePopup] = useState(false);
  //Состояние выбраного цвета
  const [selectedColor, setSelectedColor] = useState(null);
  //Состояние поля добавления списка задач
  const [inputValue, setInputValue] = useState("");
  //Состояние загрузки
  const [isLoading, setIsLoading] = useState(false);

  //Обнуление состояния окна добавление списка задач
  const onClose = () => {
    setInputValue("");
  ...
  const addList = () => {
    ...
    axios
      ...
      .then(({ data }) => {
        //Созданеи нового обьекта списка задач
        const hex = colors.filter((c) => c.id === selectedColor)[0].hex;
        const listObg = { ...data, color: hex, tasks: [] };
        onAdd(listObg);
        onClose();
      })
      .finally(() => {
        ...
        visiblePopup && (
          <div className="add-list__popup">
...

            {
              //кнопка добавить
              <button
                disabled={!selectedColor || !inputValue || isLoading}
                onClick={addList}
                className="button"
              >
